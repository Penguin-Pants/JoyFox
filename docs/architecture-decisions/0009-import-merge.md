...
6. **Safety.**
   - The whole file is checked first: JSON, schema version, scope, every record
     ...
     than a day in the future, no duplicate records, no two accounts with one
     identifier, and in an account export every record in that account. Any
     problem refuses the whole file.
