...
- Done (2026-09-26): rule presets (PRD Section 11.3), as V1-11 (ADR 0016).
...
| V1-2 | Compatibility Overlay: highlight the viewed profile's preferences that match the user's own on the profile page (PRD 6.2), a compatibility badge on every surface where a profile card shows (search results, the inbox, event attendee lists; PRD 8.3) and a sort-by-compatibility toggle on search results (PRD 10.1). **Built (2026-09-26):** the viewer's own preferences are read on their own profile; a "Shared preferences" section and a frame on each shared tag on other profiles; "N shared" on search cards, inbox rows and event guest lists; "Sort by shared preferences" on search (`src/extraction/preferences.ts`, `src/compatibility/`, `src/content/compatibility.ts`); live check: `manual-acceptance.md`, items 116 to 120 | E1; E3; E4 (attendee lists); D5 (answered, ADR 0016) | Every tag marked as matching is independently verifiable by reading both checklists manually (PRD 6.2). **Approved (owner, 2026-09-26):** for two profiles that share preferences, every shared preference is highlighted and no other; two loaded results with different overlaps show different results and sort in overlap order; each profile card on search results, the inbox and event attendee lists shows the same compatibility result as that member's profile page (PRD 8.3), and the sort orders the loaded search results by it. | M |
...
| V1-4 | Conversation History Search: full-text search over the user's own cached messages (PRD 6.2, 13.3). **Built (2026-09-26):** a conversation page stores the messages it shows while message caching is on; the options page's "Messages" tab has the switch, the purge window and the search (`src/messages/`, `src/extraction/messages.ts`, `src/content/message-cache.ts`, `src/options/messages-panel.ts`); live check: `manual-acceptance.md`, items 121 to 124 | E2; the message-caching toggle (ADR 0004, deferred to this caller by ADR 0015) | Cached message text is on by default, has a configurable auto-purge window, default 12 months, and is always manually deletable (PRD 13.3). **Approved (owner, 2026-09-26, ADR 0016):** with caching on, every message shown in an opened conversation is cached; then every cached message that contains the query is found, and no cached message without it is shown; with caching off, nothing is stored. The original message text is stored, so results are readable, and caching is on by default, as PRD 13.3 says. | M |
| V1-5 | Personal Event Tracker: private notes, attendance and tags on event and venue listings, and a personal calendar of tracked events (PRD 6.3, 9.1, 9.2). **Built (2026-09-26):** a notes box on event and venue pages, a filter and badges on "Dates & Events", and an "Events" tab on the options page (`src/events/`, `src/content/listing-panel.ts`, `src/content/event-list-filter.ts`, `src/options/events-panel.ts`); live check: `manual-acceptance.md`, items 111 to 115 | E4; D6 (answered, ADR 0016) | Event notes persist after the listing is removed from JoyClub's own calendar post-event (PRD 6.3). **Approved (owner, 2026-09-26):** notes, attendance and tags save and show again on event and venue listings; a filter by the user's own tags or notes shows only the matching events in JoyClub's already-loaded event list, with no new request to JoyClub (PRD 9.1, 10.1); the personal calendar lists every tracked event and filters the same way (PRD 9.1). | M |
...
| V1-10 | Cross-surface profile signals: the completeness badge, trust score, note icon and profile tags on every surface where a profile card shows (search results, the inbox, event attendee lists; PRD 8.3, 8.4), notes and tags editable inline there (PRD 6.2, 9.3), and a filter that hides incomplete profiles in the loaded search results (PRD 8.1). V1-2 covers the compatibility overlay. **Built (2026-09-26):** a JoyFox signals group on each card and a completeness line on the profile page; the note button opens an editor for the member's note and tags; "JoyFox: hide incomplete profiles" on search (`src/signals/`, `src/content/card-signals.ts`, `src/content/card-note-editor.ts`, `src/content/member-cards.ts`); live check: `manual-acceptance.md`, items 125 to 128 | E1; E4 (attendee lists); D8 (answered: fewer than 3 photos or fewer than 50 words is incomplete) | The signals render the same way on every surface where a profile card shows up (PRD 8.3). A note or tag attached to a profile ID follows that profile everywhere, not just where it was created (PRD 8.4). The note icon is editable inline wherever it appears (PRD 6.2). **Approved (owner, 2026-09-26, ADR 0016):** for one member, each signal on a search-result card, an inbox row and an attendee-list entry matches the same signal on that member's profile page; a tag added from a search result shows on that member's attendee-list entry; a tag and a note, each created and then edited from an attendee-list entry, save to that member's profile ID and show on the profile page; for a member whose photo count, word count and verification are known, the completeness badge shows those values (PRD 6.2); with one loaded result below the D8 completeness level (3 photos and 50 words) and one at or above it, the badge marks only the first as incomplete, and the incomplete-profile filter hides only that one, with no new request to JoyClub. A member whose completeness JoyFox does not know stays visible, marked unknown. | M |
...
| V1-13 | Shared-event exception: store the attendees that a tracked event's page shows, and a configurable triage exception for senders who share a tracked event with the user (PRD 7.4, 9.3). `EventMetadata` has no attendee field yet (`src/domain/types.ts`). "Previously met" is already the "Personally known" rule condition (ADR 0015) | V1-5; E4 | The extension reads and locally caches whatever attendee information JoyClub's own event page already renders to the logged-in viewer (PRD 9.3). A shared event co-attendee gets a configurable, but not automatic, exception; every exception is per-sender and reversible (PRD 7.4). **Approved (owner, 2026-09-26, ADR 0016):** opening a tracked event's page stores the shown attendees' profile IDs with that event, and that member's profile page lists the event; the exception is off by default; with it on, a sender who appears on a tracked event that the user marked as Attending or Attended is placed Qualified, the "Why" panel names the event, and the user can undo it for that sender. An event's attendee list is kept while the event is tracked and deleted when it is no longer tracked. | M |
| V1-9 | Self-distributed public release on GitHub (PRD 18.2, 20) | V1-2 to V1-5, V1-7, V1-8 and V1-10 to V1-13 (V1-1 is dropped; V1-6 is deferred); D7 (answered: no fixed testing period, ADR 0016); MVP release gate (build plan Section 28); F8 (channel chosen: unlisted, automatic updates; verification checklist open); D3 (answered); D4 (answered: disclaimer, ADR 0016) | **Proposed:** the GitHub repository and its release are public and hold the source under the chosen license (D3); the release page repeats the README's disclaimer (D4); the signed build attached to the release installs on release Firefox and passes the MVP release gate; an installed release updates itself to a newer version through the GitHub-hosted `update_url` (F8); building the release tag's public source with the documented command gives files identical to the ones in the signed `.xpi`, apart from the signature files (reproducible builds, PRD 13.2). | M |

### Blocked on site evidence

- **E1:** captured 2026-09-26 (`live-evidence/11-search.md`). The results URL
...
- **E2:** captured 2026-09-26 (`live-evidence/12-messages.md`). Each message
  ...
  so E2 is complete. V1-4 is built from it, with the message-caching switch
  (ADR 0004, ADR 0016).
- **E3:** structure captured 2026-09-26 (`live-evidence/13-preferences.md`):
  ...
  the full tag vocabulary. It is not needed: tags are matched by label, and D5
  says which levels count as shared. V1-2 is built from it.
- **E4:** captured 2026-09-26 (`live-evidence/14-events.md`,
  `15-venues.md`): the event list (event ID in `data-element-id`), the event
  page, the attendee list (visible to the owner; each entry links to a
...
- **D1:** closed as moot (owner, 2026-09-26, ADR 0016): it only decided which
  audiences V1-1 needed, and V1-1 is dropped.
- **D2:** deferred with V1-6 (owner, 2026-09-26, ADR 0016). When sync is
  built: a simple REST endpoint, WebDAV, or something like remoteStorage (PRD
...
- **D7:** answered (owner, 2026-09-26, ADR 0016): no fixed personal testing
  period. The release follows when V1 is done and the MVP release gate passes,
  against the recommended 4 weeks plus exit criteria.
- **D8:** answered (owner, 2026-09-26, ADR 0016). "Complete profiles only": at
  least 3 photos and 50 words of profile text. "High-trust members": verified,
  ...
  completeness level.
- The proposed acceptance criteria above: all approved (V1-2 to V1-5, V1-7,
  V1-8 and V1-10 to V1-13); V1-9's are still proposed.
...
3. Evidence captured (E1 to E4, 2026-09-26). V1-5 is built; its live checks
   are items 111 to 115. V1-2 is built; its live checks are items 116 to 120.
   V1-4 is built; its live checks are items 121 to 124. V1-10 is built; its
   live checks are items 125 to 128. Next (approved, owner, 2026-09-26): V1-13.
