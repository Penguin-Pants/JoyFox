...
| F9 | Done | Matrix in `08-attribute-matrix.md`. Verification and gender appear on all three pages, photo count only on the profile, profile text on the conversation and the profile, and join date nowhere. |
...
- Decide what M1 and M4 do about account age, which JoyClub never shows.
