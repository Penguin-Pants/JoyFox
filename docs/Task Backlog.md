...
| F7 | Partial | Ignore is not in the conversation's "Optionen" menu; it is "Profil ignorieren" in the profile's menu (`profile-context-menu`), which opens a confirmation dialog (`live-evidence/10-ignore.md`, 2026-09-24). M9 is therefore Path B. The result after confirming and the Delete details remain (item 7). |
