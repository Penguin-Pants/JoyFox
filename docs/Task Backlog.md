...
| V1-11 | Rule presets: Open, Complete profiles only, Verified members, High-trust members and Custom (PRD 11.3) | M4's rule builder (built); D8 (answered, ADR 0016) | The presets mirror the standalone spec's presets (PRD 11.3). **Proposed:** each preset fills the builder with the conditions ADR 0016 lists (from PRD 11.3 and the D8 values); the user can then edit the result like any other rule; Custom opens the builder with no conditions set. | S |
| V1-12 | Configurable `ProfileSnapshot` retention (PRD 13.3). Today `PROFILE_SNAPSHOT_RETENTION` in `src/storage/repositories.ts` keeps the newest 20 per member, with no setting | None | The default keeps the latest snapshot plus a short history window, configurable, to bound how much sensitive data sits at rest (PRD 13.3). **Approved (owner, 2026-09-26, ADR 0016):** a setting controls how many snapshots are kept per member, default 20 (the current value); after the user lowers it, each member keeps only that many of the newest snapshots, including the ones already stored; the latest snapshot is always kept. | S |
...
- **D8:** answered (owner, 2026-09-26, ADR 0016). "Complete profiles only": at
  ...
  completeness level.
- The proposed acceptance criteria above (V1-7, V1-8 and V1-12 are approved).
...
1. Ready now: V1-7, V1-8 and V1-12 (criteria approved, ADR 0016).
2. Ready once its proposed criteria are approved: V1-11 (D8 is answered).
3. Ready once its decision is made: V1-6 (D2).
4. Blocked on evidence, decisions or other tasks: V1-3 (E1), V1-4 (E2), V1-2
   (E1, E3, E4, D5), V1-5 (E4, D6), V1-10 (E1, E4), V1-13 (V1-5, E4).
5. Last: V1-9, after V1-2 to V1-8 and V1-10 to V1-13, the personal dogfooding
   period (D7), the MVP release gate, F8 and D4.
